...
- **Notes on a submission.** Editors can leave notes on the submission detail view ("called them back", "quote sent"), with author and date, and optionally email a note to the notification recipients (`_emails/note.twig`, overridable per site). Notes are cleared when a submission is anonymized — a note may quote exactly the personal data being removed.
- **One-time submit token** (opt-in, under Automatic bot checks): every rendered form gets a token that is accepted exactly once, so a captured request cannot be replayed with varied content — the true replay guard on top of the existing double-submit dedupe. Replays are kept as reviewable spam; a forged token is rejected outright. Not for sites that cache full pages — every visitor would share one token.
- New setting **Spam behaviour**: keep pretending success to flagged visitors (default, bots learn nothing) or show an error naming the spam reason — a debug aid for diagnosing false positives together with the affected visitor.
- New setting **Spam retention (days)**: prune stored spam sooner than regular submissions. Plugin-wide and always a hard delete; the per-form retention overrides keep governing real submissions.
...
- **Stencils.** The import screen now offers ready-made starters — Contact, Quote request, and Booking-with-payment (prices included) — one click creates the form, everything editable afterwards.
- **Plugin settings reorganized** from six tabs to five, grouped by the question an admin arrives with: Spam protection (captcha + the always-on local checks together), Emails (the notification language, no longer hiding under "Storage", plus a pointer to the per-form mail settings), Storage & retention (merged — retention was a one-field tab), Uploads, Payments. All values survive the move.
- **Form edit screen reorganized.** Tabs ordered by how often editors touch them: Fields / Notification / Confirmation / After submit / Payment / Form settings. Payment moved out of the settings pane into its own tab (only shown when a provider is configured); the settings pane is back to name, handle, embed and retention.
