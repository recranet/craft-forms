...
- The Notification tab's **extra notifications** (`extraNotification` macro + `#rf-extra-template`, `extraNotifications[N][…]` inputs) reuse the SAME `conditionsUI()` component as routing-rules editor — its serialize() must keep tolerating editors outside a field card. Rows normalize via `Forms::normalizeExtraNotifications()`
