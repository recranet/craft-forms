...
- `settings.twig` — plugin settings, five tab panes (spam protection / emails / storage & retention / uploads / payments; `rfs-` prefix, all inputs stay in the DOM so one form posts everything). The spam pane keeps provider + keys in view (provider-dependent blocks via `data-rfs-show="<provider values>"` — hidden inputs still post, so switching providers never wipes stored keys); everything with sane defaults sits in collapsed `<details class="rfs-group">` sections (Scoring / Failure handling / Automatic bot checks / Blocklist) that auto-open on validation errors. The emails pane points editors to the per-form mail settings. Craft namespaces the form: the select is `settings[captchaProvider]`
