...
- **Conditional fields**: a field row may carry an optional `conditions` key — `{mode: 'all'|'any', rules: [{field: <uid of another field>, operator: is|isNot|contains|isEmpty|isNotEmpty, value: <string>}]}`; absent/empty = always visible. Evaluation lives in two places that must stay in parity: `src/rules/RuleEvaluator.php` (server: `applyPost()` discards hidden-field values, `validateFormData()` skips hidden fields entirely — a hidden field is **never** required) and the inline `{% js %}` block in `src/templates/_render/form.twig` (front end: `hidden` attribute + inputs disabled + `required` lifted). Comparison is trimmed/case-insensitive strings, checkboxes normalize to `'1'`/`''`, and everything malformed fails open (visible) — a broken rule must never lock a visitor out.
