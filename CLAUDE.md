...
- **Spam pipeline** (`src/services/SpamService.php`, order matters): blocklist → honeypot → submit timing → captcha with token binding (action + hostname). Captcha providers live in `src/captchas/` behind `CaptchaInterface` (reCAPTCHA v2/v3/Enterprise, Turnstile); provider choice + thresholds in `Settings`. Verdicts (`SpamVerdict`): pass / reviewable spam (stored + flagged, visitor sees success, no email) / **reject** (honeypot, forged timestamp, score below reject threshold — not even stored). A `CaptchaError` (bad keys, provider down) is a config problem — **never spam**; fail-open default accepts + flags. Never collapse error into spam — that's the entire reason this plugin exists.
