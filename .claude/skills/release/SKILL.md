...
1. Working tree clean, on `main`, all changes committed.
2. Lint: `find src -name '*.php' -print0 | xargs -0 -n1 php -l | grep -v "No syntax"` — must be silent.
3. Pick the version (semver):
...
4. Any DB schema change needs a migration in `src/migrations/` **and** a `schemaVersion` bump in `src/Plugin.php` — Craft only runs plugin migrations when `schemaVersion` increases.
5. Tag and push:
...
6. Packagist auto-updates via the GitHub hook. Verify:
...
7. In a consuming project: `ddev composer update recranet/craft-forms && ddev craft up`.
